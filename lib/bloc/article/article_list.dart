...
import 'dart:async';
import 'package:freezed_tutorial/data/models/models.dart';
import 'package:freezed_tutorial/data/repositories/repositories.dart';
...
class ArticleListState with _$ArticleListState {
  ...
  // Данные получены
  const factory ArticleListState.loaded(ArticleList dataList) =
      LoadedArticleListState;
...
class ArticleListBLoC extends Bloc<ArticleListEvent, ArticleListState> {
  ArticleListBLoC(this._articleRepository)
      : super(const InitialArticleListState());
  ...
  Stream<ArticleListState> _load({bool loadMore = false}) async* {
    ...
    try {
      yield ArticleListState.loaded(await _articleRepository
          .fetchList(loadMore: loadMore)
